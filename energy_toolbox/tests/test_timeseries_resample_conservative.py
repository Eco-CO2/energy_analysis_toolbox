...
def test_deduce_flows_nominal():
    ...
    flow_rates = deduce_flows(volumes, last_step_duration=10.)
...
def test_flow_rate_out_of_bounds_right():
    ...
    sources = pd.Series(
        np.arange(0, 10),
        pd.date_range(pd.Timestamp("2021-12-15"), periods=10, freq="45min"))
    target_instants = pd.date_range(pd.Timestamp("2022-12-16"), periods=10, freq="12min")
    # default 0 padding
...
def test_flow_rate_right_bound_overlap():
    ...
    compare_flow_rates(interp,
                       pd.Series([3., 1.],
                                 index=target_instants))
...
def test_flow_rate_out_of_bounds_left():
    ...
    sources = pd.Series(
        np.arange(0, 10),
        pd.date_range(pd.Timestamp("2021-12-15"), periods=10, freq="45min"))
    target_instants = pd.date_range(pd.Timestamp("2020-12-14"), periods=10, freq="12min")
    # default 0 padding
    interp = flow_rate_conservative(sources, target_instants)
    compare_flow_rates(interp, pd.Series(np.zeros(10), index=target_instants))
...
def test_flow_rate_identity():
    ...
    sources = pd.Series(
        ...
        pd.date_range(pd.Timestamp("2021-12-15"), periods=10, freq="45min"))
    compare_flow_rates(
        flow_rate_conservative(sources, sources.index),
        sources)
...
def test_flow_rate_conservative():
    ...
    expected_fr_values = np.array([
        ...
        flow_rates.iloc[1],
        flow_rates.iloc[1],
        (flow_rates.iloc[2] * 2 + flow_rates.iloc[3] * 1) / 3, # dur_3 is half dur_2
...
def test_volume_conservative_out_of_bounds_right():
    ...
    sources = pd.Series(
        np.arange(0, 10),
        pd.date_range(pd.Timestamp("2021-12-15"), periods=10, freq="45min"))
    target_instants = pd.date_range(pd.Timestamp("2022-12-15"), periods=10, freq="12min")
...
def test_volume_conservative_out_of_bounds_left():
    ...
    sources = pd.Series(
        np.arange(0, 10),
        pd.date_range(pd.Timestamp("2021-12-15"), periods=10, freq="45min"))
    target_instants = pd.date_range(pd.Timestamp("2020-12-15"), periods=10, freq="12min")
    # default 0 padding
    interp = volume_conservative(sources, target_instants)
    compare_volumes(interp, pd.Series(np.zeros(10), index=target_instants))
...
def test_volume_conservative_identity():
    ...
    compare_volumes(
        volumes,
        volume_conservative(volumes, volumes.index))
...
def test_volume_conservative_1():
    ...
    volumes_expect = pd.Series(
        [0.025, 0.025],
        index=pd.DatetimeIndex([t0 + dt / 2, t0 + 3 * dt /2])
        )
    compare_volumes(
        volumes_expect,
        volume_conservative(volumes, volumes_expect.index))
...
def test_volume_conservative_finer_2():
    ...
    volumes = pd.Series(
        [0.040, 0.060, 0.050],
        index=pd.date_range(t0, t0 + 3 * dt, inclusive='left',
                            freq='300s'),
        )
    volumes_expect = pd.Series(
...
def test_volume_conservative_ordinary():
    ...
    volumes_expect = pd.Series(
        [0., 0., 0.030, 0.600, 2.970, 0.009, 0.072],
        index=pd.DatetimeIndex([
            ...
            # 0.009 m3 here
            pd.Timestamp("2022-02-23 23:45:22"), # 2h later, 15 min after the new consumption
    ...
    volumes_obtained = volume_conservative(volumes,
...
def test_volume_conservative_coarser():
    ...
    assert (coarse_volumes.loc["2020-01-06 12:00":].sum()
            == pytest.approx(volumes.loc["2020-01-06 12:00":].sum()))
...
def test_volume_conservative_as_integral():
    ...
    volumes = example_volume_one_day("2020-01-06")
    coarse_ixs = pd.date_range(
        start="2020-01-05", end="2020-01-07", periods=1)
    coarse_volumes = volume_conservative(volumes, coarse_ixs,
                                         last_target_step_duration=DAY * 3)
