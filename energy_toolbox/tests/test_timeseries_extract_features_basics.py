...
def test_evenly_spaced_durations():
    ...
    durs = timestep_durations(series, last_step=42.)
    assert durs.iloc[-1] == 42.
...
def test_intervals_over():
    ...
    power = pd.Series(np.array([0, 1, 1.5, 2., 2., 3., 1.5, 0.]),
                      index=time_range)
    start_f = "start"
    end_f = "end"
    up_loc, up_iloc = intervals_over(
        power,
        low_tshd=1.5,
...
def test_intervals_over_no_measures():
   ...
   """
   power = pd.Series([])
   assert intervals_over(power, low_tshd=0.).empty
...
def test_intervals_over_no_interval_noisy(timestep):
    ...
    max_noise = 50.
    fake_data = np.abs(np.random.normal(0, 10., n_steps))
    fake_data = np.where(fake_data < max_noise, fake_data, max_noise)
    fake_series = pd.Series(fake_data, index=time_range)
    assert intervals_over(fake_series, low_tshd=max_noise + 1.).empty
