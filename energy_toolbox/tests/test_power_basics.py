...
def _intervals():
    intervals = pd.DataFrame.from_dict({
        'start': [pd.Timestamp('2023-10-29 00:00', tz='Europe/Paris'),
                  pd.Timestamp('2023-10-30 02:00', tz='UTC+02:00'),
                  ],
        'end': [pd.Timestamp('2023-10-29 01:00', tz='Europe/Paris'),
...
def test_integrate_over():
    ...
    expected = pd.Series([3600., 3600.], index=['A', 'B'])
...
def test_integrate_over_not_in_index():
    ...
    expected = pd.Series([7200., 7200.], index=['A', 'B'])
    pd.testing.assert_series_equal(
        integrate_over(intervals, _constant_power()),
        expected,
    )
