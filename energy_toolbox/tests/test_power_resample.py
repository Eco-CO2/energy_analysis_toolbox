...
def test_to_freq_one_to_many():
    ...
    with pytest.raises(ETUndefinedTimestepError):
        new_power_series = to_freq(power_series, new_freq)
    new_power_series = to_freq(power_series, new_freq, last_step_duration=pd.Timedelta(freq).seconds)
...
def test_to_freq_two_to_many():
    ...
    """
    power_series = pd.Series([1.0, 1.0],
    ...
    new_power_series = to_freq(power_series, new_freq, last_step_duration= pd.Timedelta(freq).seconds / 2)
...
def test_to_freq_many_to_one():
    ...
    assert new_power_series.sum() * pd.Timedelta(new_freq).seconds == pd.Timedelta(freq).seconds*30
    # last step duration is force to half the new period
    new_power_series = to_freq(power_series, new_freq, last_step_duration=pd.Timedelta(freq).seconds / 2)
    assert len(new_power_series) == 1
    assert new_power_series.sum() * pd.Timedelta(new_freq).seconds == pd.Timedelta(freq).seconds * 29.5

def test_to_freq_one_to_one():
    """Test case when the input series has length 1 and is resampled to one element.
    """
    freq="1min"
    power_series = pd.Series([1.0] ,
                             index=pd.date_range(start="2020-01-01", periods=1, freq=freq))
    new_freq = "1h"
    new_power_series = to_freq(power_series, new_freq, last_step_duration= pd.Timedelta(freq).seconds)
    assert new_power_series.index.freq == pd.Timedelta(new_freq)
    assert new_power_series.index[0] == pd.Timestamp("2020-01-01")
    assert len(new_power_series) == 1
    assert new_power_series.sum() * pd.Timedelta(new_freq).seconds == pd.Timedelta(freq).seconds
