...
import numpy as np
import pytest
from energy_toolbox.thermosensitivity import (
    ThermoSensitivity,
...
from energy_toolbox.synthetic.thermosensitive_consumption import (
...
class TestThermoSensitivity:
    ...
    def test_fit_only_heating_no_noise(self):
        ...
        ts.fit()
        assert ts.model.params["heating_degree_days"] == pytest.approx(self.ts_heating, rel=1e-1)
        assert ts.model.params["Intercept"] == pytest.approx(self.expected_intercept, rel=1e-1)
        with pytest.raises(KeyError):
            ts.model.params["cooling_degree_days"]

    def test_fit_only_cooling_no_noise(self):
        """Test the ThermoSensitivity class."""
        data = self.synth_cooling_only.random_consumption(size=self.data_generated_size, start="2022-07-01")
        ...
        ts.fit()
        assert ts.model.params["cooling_degree_days"] == pytest.approx(self.ts_cooling, rel=1e-1)
        assert ts.model.params["Intercept"] == pytest.approx(self.expected_intercept, rel=1e-1)
        with pytest.raises(KeyError):
    ...
    def test_fit_heating_and_cooling_no_noise(self):
        ...
        ts = ThermoSensitivity(
            energy_data=data["energy"],
            temperature_data=data["T"],
            degree_days_base_temperature={"heating": self.tref_heating, "cooling": self.tref_cooling},
            degree_days_computation_method="mean",
    ...
    def test_only_heating_no_noise_tref_calibration(self):
        ...
        assert ts.degree_days_base_temperature["heating"] == pytest.approx(self.tref_heating, rel=1e-1)
        with pytest.raises(KeyError):
    ...
    def test_only_cooling_no_noise_tref_calibration(self):
        ...
        ts.calibrate_base_temperatures()
        assert ts.degree_days_base_temperature["cooling"] == pytest.approx(self.tref_cooling, rel=1e-1)
        with pytest.raises(KeyError):
    ...
    def test_aggregted_data(self):
        ...
        assert set(agg_data.columns) == {"energy", "temperature", "heating_degree_days"}

        ts.degree_days_type = "cooling"
        ts._aggregate_data({"cooling": self.tref_cooling})
        agg_data = ts.aggregated_data
        assert set(agg_data.columns) == {"energy", "temperature", "cooling_degree_days"}

        ts.degree_days_type = "both"
        ts._aggregate_data({"heating": self.tref_heating, "cooling": self.tref_cooling})
        agg_data = ts.aggregated_data
        assert set(agg_data.columns) == {"energy", "temperature", "heating_degree_days", "cooling_degree_days"}
    ...
    def test_calculate_degree_days(self):
        ...
        degree_days = ts._calculate_degree_days({"heating": self.tref_heating, "cooling": self.tref_cooling})
        assert isinstance(degree_days, pd.DataFrame)
        assert degree_days.index.equals(ts.resampled_temperature.index)
        assert set(degree_days.columns) == {"heating_degree_days", "cooling_degree_days"}
    ...
    def test_calibrate_base_temperature(self):
        ...
        tref = ts.calibrate_base_temperature(dd_type="heating",
                                              t0 = 12,
                                              xatol=1e-3,)
        assert tref == pytest.approx(self.tref_heating, rel=1e-1)
        tref = ts.calibrate_base_temperature(dd_type="cooling",
                                              t0 = 30,
                                              xatol=1e-3,)
        assert tref == pytest.approx(self.tref_cooling, rel=1e-1)
        with pytest.raises(ValueError):
            ts.calibrate_base_temperature(dd_type="both",
                                           t0 = 30,
                                           xatol=1e-3,)
...
class TestCategoricalThermoSensitivity():
    ...
    parameters = [{"base_energy": 100, "ts_heat": 2, "ts_cool": 1, "noise_std": 0.0001},
    ...
    def setup_method(self):
        ...
        self.synth = CategorySynthTSConsumption(category_func=self.category_func,
    ...
    def test_fit(self):
        ...
        model = ts.model
        np.testing.assert_allclose(model.params["heating_degree_days:weekend"], self.parameters[1]["ts_heat"], rtol=1e-1)
        np.testing.assert_allclose(model.params["heating_degree_days:weekday"], self.parameters[0]["ts_heat"], rtol=1e-1)
        np.testing.assert_allclose(model.params["cooling_degree_days:weekend"], self.parameters[1]["ts_cool"], rtol=1e-1)
        np.testing.assert_allclose(model.params["cooling_degree_days:weekday"], self.parameters[0]["ts_cool"], rtol=1e-1)
        np.testing.assert_allclose(model.params["Intercept:weekday"], self.parameters[0]["base_energy"], rtol=1e-1)
        np.testing.assert_allclose(model.params["Intercept:weekend"], self.parameters[1]["base_energy"], rtol=1e-1)
...
class TestDailyCategoricalThermoSensitivity():
    ...
    def category_func_series(index):
            ...
            np.array
                The categories
            """
            return pd.Series(np.where(index.dayofweek < 5, "weekday", "weekend"), index=index)
...
class TestDayOfWeekCategoricalThermoSensitivity():
    ...
    def setup_method(self):
        ...
        self.synth = CategorySynthTSConsumption(category_func=self.category_func,
                                            ...
                                            t_ref_cool=self.tref_cooling,
                                            t_ref_heat=self.tref_heating,
                                            list_categories=["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]
                                            )
    ...
    def test_fit(self):
        ...
        model = ts.model
        for idex, dayname in enumerate(["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]):
            np.testing.assert_allclose(model.params[f"heating_degree_days:{dayname}"], self.parameters[idex]["ts_heat"], rtol=1e-1)
            np.testing.assert_allclose(model.params[f"cooling_degree_days:{dayname}"], self.parameters[idex]["ts_cool"], rtol=1e-1)
            np.testing.assert_allclose(model.params[f"Intercept:{dayname}"], self.parameters[idex]["base_energy"], rtol=1e-1)

class TestAutoCategoricalThermoSensitivity():
    parameters = [
                  {"base_energy": 100, "ts_heat": 1, "ts_cool": 2, "noise_std": 0.00001},  # class 1
                  {"base_energy": 100, "ts_heat": 2, "ts_cool": 4, "noise_std": 0.00001},  # class 2
                  {"base_energy": 100, "ts_heat": 1, "ts_cool": 2, "noise_std": 0.00001},  # class 1
                  {"base_energy": 100, "ts_heat": 4, "ts_cool": 8, "noise_std": 0.00001},  # class 3
                  {"base_energy": 100, "ts_heat": 4, "ts_cool": 8, "noise_std": 0.00001},  # class 3
                  {"base_energy": 100, "ts_heat": 2, "ts_cool": 4, "noise_std": 0.00001},  # class 2
                  {"base_energy": 100, "ts_heat": 1, "ts_cool": 2, "noise_std": 0.00001},  # class 1
                  ]
    ...
    def test_fit(self):
        ...
        model = ts.model
        for idex, dayname in enumerate(["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]):
            np.testing.assert_allclose(model.params[f"heating_degree_days:{dayname}"], self.parameters[idex]["ts_heat"], rtol=2e-1)
            np.testing.assert_allclose(model.params[f"cooling_degree_days:{dayname}"], self.parameters[idex]["ts_cool"], rtol=2e-1)
            np.testing.assert_allclose(model.params[f"Intercept:{dayname}"], self.parameters[idex]["base_energy"], rtol=2e-1)
    ...
    def test_merge_and_fit(self):
        ...
        model = ts.model
        daynames = ["Monday-Wednesday-Sunday", "Tuesday-Saturday", "Thursday-Friday"]
        indexes = [0, 1, 3]
        print(model.params)
