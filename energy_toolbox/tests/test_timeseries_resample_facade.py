"""Tests for :py:mod:`energy_toolbox.timeseries.resample._facade` module.
"""
import pandas as pd
import numpy as np
import pytest
...
def test_to_freq_volume_conservative():
    ...
    # oversampling
    pd.testing.assert_series_equal(
        to_freq(example_series, '30min', method='volume_conservative',
...
def test_to_freq_flow_rate_conservative():
    ...
    # oversampling
    pd.testing.assert_series_equal(
        to_freq(example_series, '30min', method='flow_rate_conservative',
                last_step_duration=3600 * 2,
