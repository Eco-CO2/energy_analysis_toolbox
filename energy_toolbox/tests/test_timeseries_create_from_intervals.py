...
def test_flatten_one_interval():
    ...
    test_data = pd.DataFrame([[start, end, 42.]],
                             columns=[CTK.start_f, CTK.end_f, 'test'])
    flat_consumption = flatten_and_fill(
        test_data, start_f=CTK.start_f, end_f=CTK.end_f, time_f=CTK.time_f)
    expect_index = pd.DatetimeIndex([start, end],
                                    name=CTK.time_f)
    expect_series = pd.DataFrame(
        np.array([42., np.nan]).reshape(2, 1),
        index=expect_index,
        columns=['test'],
    )
    pd.testing.assert_frame_equal(expect_series, flat_consumption,
                                  check_dtype=False,
                                  check_freq=False)
    pass
...
def test_flatten():
    ...
    starts = pd.date_range(start=begin, freq='10min',
                           periods=3, name=start_f)
    targets = pd.date_range(start=begin, freq='5min',
                            periods=6, name=time_f)
    data = {
        start_f : starts,
        end_f: starts + pd.Timedelta('5min'),
        'col_str': ['toto'] * 3,
        'col_float': np.arange(0., 6., 2.),
        }
    ...
    # explicit filling
    flat_filled_table = flatten_and_fill(table,
                                         fill_values={'col_str': 'tata'},
                                         end_f=end_f, start_f=start_f)
    flat_expect['col_str'] = ['toto', 'tata'] * 3
    pd.testing.assert_frame_equal(flat_expect,
                                  flat_filled_table,
                                  check_freq=False,
                                  check_dtype=False,
                                  )
