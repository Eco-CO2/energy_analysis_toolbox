...
from energy_toolbox.timeseries.profiles.preprocessing.history_filters.weekdays import (
    weekdays_only,
    weekends_only,
    same_day_only,
    )
...
def test_same_day():
    ...
    history = sinusoid_history_df(start=monday, n_days=21, freq='1D',
...
def test_weekdays():
    monday = pd.Timestamp("2022-01-03")
    history = sinusoid_history_df(start=monday, n_days=7, freq='1D',
                                  period_variation=7 * SK.day)
    # on df
...
def test_pipelining():
    ...
    pd.testing.assert_frame_equal(filtered, history.iloc[[0, 7], :],
                                  check_freq=False)
