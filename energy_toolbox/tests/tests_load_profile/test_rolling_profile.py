...
import pandas as pd
import numpy as np
from energy_toolbox.timeseries.profiles.rolling_profile import RollingProfile, RollingQuantileProfile
...
def arange_multicolumn(n_days, step, r_rows=10):
    ...
    """
    return pd.DataFrame(
        np.arange(0, n_days * r_rows * step, step)
        .reshape(r_rows, n_days)
        )
...
def expected_constant_df(value, index, window_size):
    ...
    """
    expected_df = pd.DataFrame(data= value,
                               index=index,
                               columns=["value"]
                               )
    return expected_df.rolling(window_size,
                               center=True,
                               ).median()


@pytest.mark.parametrize("n_rows, step, window_size, ddof", product([1,3,4],  # number of columns
...
def test_windowed_rolling_agg_many_combinations(n_rows, step, window_size, ddof):
    ...
    rolling_profiler = RollingProfile(window=window_size,
                                      aggregation=lambda x:np.std(x, ddof=ddof)
                                      )
...
def test_mean_offset(n_days, step, window_size, freq_minutes):
    ...
    df = pd.DataFrame(data={"value":data.to_numpy().T.ravel()},
                      index=pd.date_range(start="2022-09-23 00:00",
                                          periods= n_rows*n_days,
                                          freq=f"{freq_minutes}min",
                                          ),
    )
    expected_value = np.mean(data.iloc[0, :]) + np.std(data.iloc[0, :])
    expected_time = pd.date_range(start="2022-09-23 00:00",
...
@pytest.mark.parametrize("n_days, step, window_size, q", product([1,3,4],  # number of columns
                                                                 [1,2,3],    # step in the data range
                                                                 [3,4,7],  # size of the rolling window
...
def test_RollingQuantileProfile_many_combinations(n_days, step, window_size, q):
    ...
    data = np.array(
        np.arange(0, n_days*step, step ).tolist()
        * (r_rows//n_days)
        ).reshape(-1, n_days).T.ravel()
    index_name = "timestamp_with_changes"
    index = pd.DatetimeIndex(data=pd.date_range(start = "2022-09-23 00:00",
                                                periods=r_rows,
                                                freq=f"{freq_minutes}min",
                                                name=index_name
                                                )
    ...
    expected_value = np.quantile( np.array(np.arange(0, n_days*step, step ).tolist()*(window_size)), q=q)
    expected_time = pd.date_range(start = "2022-09-23 00:00",
                                                 end="2022-09-23 23:59",
                                                 freq=f"{freq_minutes}min",
                                                 name=index_name,
...
@pytest.mark.parametrize("n_days, freq, tz", product([1,3,4],  # number of days
...
def test_daily_pivot_many_combinations(n_days, freq, tz):
    ...
    history = pd.DataFrame(data=np.ones(len(index)),
                           index=index,
                           columns=["value"])
