...
import pandas as pd
import numpy as np
from energy_toolbox.timeseries.profiles.localization import (LocalizedMeanProfile,
...
def test_localized_mean_profile():
    ...
    index = pd.DatetimeIndex(data=pd.date_range(start=start,
                                                end=end,
                                                freq=str(freq_minutes)+"min")
                             )
    data = np.random.randn(len(index))
    history = pd.DataFrame(data={"value": data},
                           index=index,
                           )
    time = pd.Timestamp("2022-10-23 00:00")
    expected = MeanProfile().compute(history=history,
                                     time=time)
    result = LocalizedMeanProfile().compute(history=history,
                                            time=time)
    #1.
...
def test_localized_rolling_profile():
    ...
    time = pd.Timestamp("2022-10-23 00:00")
    agg = np.std
    expected = RollingProfile(window, agg).compute(
        history=history, time=time)
    result = LocalizedRollingProfile(window,agg).compute(history=history,
                                            time=time)
    #1.
    pd.testing.assert_frame_equal(expected, result)
    history.index = index.tz_localize("Europe/Paris")
...
def test_localized_rolling_quantile_profile():
    ...
    time = pd.Timestamp("2022-10-23 00:00")
    expected = RollingQuantileProfile(window,q).compute(history=history,
                                     time=time)
    result = LocalizedRollingQuantileProfile(window,q).compute(history=history,
                                            time=time)
    #1.
    pd.testing.assert_frame_equal(expected, result)
    history.index = index.tz_localize("Europe/Paris")
