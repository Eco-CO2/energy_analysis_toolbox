...
from energy_toolbox.timeseries.profiles.preprocessing.history_filters.categories import (
    same_category,
    keep_categories,
    remove_categories,
    )
...
def is_working(timestamp, off_days=None):
    ...
    if off_days is None:
        off_days = []
    return not (timestamp.dayofweek in off_days)
...
def test_same_category_identity():
    ...
    history = sinusoid_history_df(start=monday, n_days=14, freq='1D',
                                  period_variation=7 * SK.day)
    # on df
    filtered = same_category(history, date=monday + pd.Timedelta('1D'),
                             classificator=single_category)
    pd.testing.assert_frame_equal(filtered, history)
    # on series
    filtered_s = same_category(history.iloc[:,0], date=monday + pd.Timedelta('1D'),
                               classificator=single_category)
...
def test_same_category_arbitrary():
    ...
    pd.testing.assert_frame_equal(
        filtered,
        history.loc[categories.values[:-1]])
    # on series
    filtered_s = same_category(history.iloc[:,0], date=monday + pd.Timedelta('1D'),
                               classificator=classificator)
...
def test_keep_categories_constant():
    ...
    history = sinusoid_history_df(start=monday, n_days=14, freq='1D',
                                  period_variation=7 * SK.day)
    # on df
    filtered = keep_categories(history, keep=[1],
                               classificator=single_category)
    pd.testing.assert_frame_equal(filtered, history)
    filtered_out = keep_categories(history, keep=[0],
                                   classificator=single_category)
    assert filtered_out.empty
    # on series
    filtered_s = keep_categories(history.iloc[:,0], keep=[1],
                                 classificator=single_category)
    filtered_out_s = keep_categories(history.iloc[:,0], keep=[0],
                                     classificator=single_category)
    pd.testing.assert_series_equal(filtered_s, history.iloc[:, 0])
...
def test_remove_categories_constant():
    ...
    history = sinusoid_history_df(start=monday, n_days=14, freq='1D',
                                  period_variation=7 * SK.day)
    # on df
    filtered = remove_categories(history, remove=[0],
                                 classificator=single_category)
    pd.testing.assert_frame_equal(filtered, history)
    filtered_out = remove_categories(history, remove=[1],
                                     classificator=single_category)
    assert filtered_out.empty
    # on series
    filtered_s = remove_categories(history.iloc[:,0], remove=[0],
                                   classificator=single_category)
    filtered_out_s = remove_categories(history.iloc[:,0], remove=[1],
                                       classificator=single_category)
    pd.testing.assert_series_equal(filtered_s, history.iloc[:, 0])
    assert filtered_out_s.empty
...
def test_keep_categories_None_classificator():
    ...
    """
    monday = pd.Timestamp("2022-01-03")
    history = sinusoid_history_df(start=monday, n_days=3, freq='1D',
                                  period_variation=7 * SK.day)
    filtered = keep_categories(history, keep=[True],
                               classificator=None)
    pd.testing.assert_frame_equal(filtered, history)
    filtered_2 = keep_categories(history, keep=None,
                                 classificator=None)
...
def test_keep_categories_working_days():
    ...
    # on series
    filtered_s = keep_categories(history.iloc[:,0], keep=[False],
                                 classificator=casa_randazzo)
    pd.testing.assert_series_equal(filtered_s, history.iloc[[0, 1, 7, 8], 0])
...
def test_keep_categories_arbitrary():
    ...
    def classificator(date):
        return categories.loc[date]
    # on df
    filtered = keep_categories(history, keep=['yes'],
                               classificator=classificator)
    pd.testing.assert_frame_equal(
        filtered,
        history.iloc[1::2])
    # on series
    filtered_s = keep_categories(history.iloc[:,0], keep=['no'],
                                 classificator=classificator)
    pd.testing.assert_series_equal(
        filtered_s,
        history.iloc[0::2, 0])
...
def test_remove_categories_None_classificator():
    ...
    history = sinusoid_history_df(start=monday, n_days=3, freq='1D',
                                  period_variation=7 * SK.day)
    filtered = remove_categories(history, keep=[True],
                                 classificator=None)
    pd.testing.assert_frame_equal(filtered, history)
    filtered_2 = remove_categories(history, keep=None,
                                   classificator=None)
    pd.testing.assert_frame_equal(filtered_2, history)
...
def test_remove_categories_working_days():
    ...
    assert remove_categories(history, classificator=casa_randazzo,
                             remove=[True, False]).empty
    # on df remove nothing
    pd.testing.assert_frame_equal(
        remove_categories(history, classificator=casa_randazzo,
                          remove=[]),
        history)
    # remove working days
    filtered = remove_categories(history, remove=[False],
                                 classificator=casa_randazzo)
    pd.testing.assert_frame_equal(
        filtered,
        history.drop(labels=history.index[[0, 1, 7, 8]])
    ...
    # on series
    filtered_s = remove_categories(history.iloc[:,0], remove=[True],
                                   classificator=casa_randazzo)
    pd.testing.assert_series_equal(filtered_s, history.iloc[[0, 1, 7, 8], 0])
...
def test_remove_categories_arbitrary():
    ...
    def classificator(date):
        return categories.loc[date]
    # on df
    filtered = remove_categories(history, remove=['no'],
                                 classificator=classificator)
    pd.testing.assert_frame_equal(
        filtered,
        history.iloc[1::2])
    # on series
    filtered_s = remove_categories(history.iloc[:,0], remove=['yes'],
                                   classificator=classificator)
    pd.testing.assert_series_equal(
        filtered_s,
        history.iloc[0::2, 0])
