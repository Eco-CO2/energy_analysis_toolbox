...
from ..timeseries.resample.interpolate import (piecewise_affine, piecewise_constant)
...
def affine_timeseries():
    ...
    nw = pd.Timestamp.now()
    inds = pd.date_range(nw, nw + pd.Timedelta(seconds=10), freq='2s')
    series = pd.Series(np.linspace(1, 11, inds.size),
                       index=inds)
    return series
...
@pytest.mark.parametrize("source_tz, targets_tz",
                         [(None, None),
                          ('Europe/Paris', None),
                          (None, 'Europe/Paris'),
...
def test_piecewise_affine(targets, expected, source_tz, targets_tz):
    ...
    sample_dts = [0., 5., 7., 10.]
    sample_values = [0., 1., 2., -1.]
    instants = pd.DatetimeIndex([start_time + pd.Timedelta(seconds=dt) for dt
                                 in sample_dts])
    bc = pd.Series(sample_values, instants)
    target_instants = pd.DatetimeIndex([start_time + pd.Timedelta(seconds=dt)
                                        for dt in targets])
...
def test_piecewise_constant_padding():
    ...
    bc = pd.Series(sample_values, instants)
    target_instants = pd.DatetimeIndex([start_time + pd.Timedelta(seconds=dt)
                                        for dt in np.array([-2, -1, 12.])])
    stair_interp = piecewise_constant(bc, target_instants)
    assert np.allclose(stair_interp.to_numpy(), np.array([0., 0., -1.]))
    stair_interp = piecewise_constant(bc, target_instants, left_pad=42.)
    assert np.allclose(stair_interp.to_numpy(), np.array([42., 42., -1.]))
