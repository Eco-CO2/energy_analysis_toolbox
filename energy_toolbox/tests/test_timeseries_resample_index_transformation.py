...
@pytest.mark.parametrize('source_tz, target_tz', [
    ('UTC', 'Europe/Paris'),
    ('Europe/Paris', 'UTC'),
    (None, 'Europe/Paris'),
    ('Europe/Paris', None),
...
def test_tz_convert_localize(source_tz, target_tz):
    ...
    pd.testing.assert_series_equal(
        tz_convert_or_localize(a_timeseries, tz=target_tz),
        expected_series
    )
    pd.testing.assert_frame_equal(
        tz_convert_or_localize(a_dataframe, tz=target_tz),
        expected_df
    )
...
def test_index_to_freq():
    ...
    freq =  "1min"
    expected_index = pd.DatetimeIndex(pd.date_range(
...
def test_estimate_timestep_two_frequency():
    ...
    timestamp_2 = list(pd.date_range(
        start=timestamp_1[-1],
        periods=10,
        freq=freq2,
        inclusive="right"
...
def test_estimate_timestep_random_frequency():
    ...
    atol = 0.1
    rtol = 0.1
    assert np.isclose(estimate_timestep(index, "median"),timestep, rtol=rtol, atol=atol)
    assert np.isclose(estimate_timestep(index, "kde"), timestep, rtol=rtol, atol=atol)
    assert np.isclose(estimate_timestep(index, "mode"), timestep, rtol=rtol, atol=atol)  # I'm quite surprised that this works !
...
def test_fill_missing_entry_series():
    ...
    fixed_series = fill_missing_entries(missing_series,
                                        sampling_period=60,
                                        security_factor=2,
                                        fill_value=42
                                        )
    pd.testing.assert_series_equal(expected_series, fixed_series,
                                   check_freq=False)
...
def test_fill_missing_entry_frame():
    ...
    fixed_frame = fill_missing_entries(missing_frame,
                                        sampling_period=60,
                                        security_factor=2,
                                        fill_value=42
                                        )
    pd.testing.assert_frame_equal(expected_frame, fixed_frame,
                                   check_freq=False)
...
def test_fill_data_holes():
    ...
    expected_series = series.copy()
    expected_series.loc[pd.Timestamp("2022-06-15 12:15")] = 42
    fixed_series = fill_data_holes(missing_series,
                                        method="median",
                                        security_factor=2,
                                        fill_value=42
                                        )
